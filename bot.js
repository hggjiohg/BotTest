...
antispam(client, {
  ...
  warningMessage: "Stop Spamming To Prevent Mute/Kick/Ban", // Warning message send to the user indicating they are going to fast.
  banMessage: "Read The Rules Next Time", // Ban message, always tags the banned user in front of it.
...
client.on('message', message => { 
  var whitelisted = "449406602286465024"// ايديك
...
if(message.content.startsWith('|at')) {
  ...
  let args = message.content.split(" ").slice(1)
  if(!args.join(" ")) return message.reply(`${prefix}at toggle/setrole [ROLE NAME]`)
  let state = args[0]
  if(!state.trim().toLowerCase() == 'toggle' || !state.trim().toLowerCase() == 'setrole') return message.reply(`Please type a right state, ${prefix}modlogs toggle/setrole [ROLE NAME]`) 
   ...
   if(!newRole) return message.reply(`${prefix}at setrole [ROLE NAME]`)
